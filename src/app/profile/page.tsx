...
import { User, Phone, MapPin, CheckCircle, ArrowLeft, ShieldAlert } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const { user, isAuthenticated, updateProfile, updateAddress } = useAuthStore();
  ...
  if (!user || !isAuthenticated) {
    return (
      <div className="py-20 max-w-md mx-auto text-center space-y-6">
        ...
        <p className="text-slate-500">Please sign in to view your profile panel dashboard.</p>
        <button
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="border border-slate-105 dark:border-zinc-850 bg-white dark:bg-zinc-900 rounded-2xl p-5 sm:p-6 space-y-4">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-4">
              ...
              <Button type="submit" isLoading={isUpdatingProfile} className="w-full py-2.5 text-sm">
          ...
          <div className="border border-slate-105 dark:border-zinc-850 bg-white dark:bg-zinc-900 rounded-2xl p-5 sm:p-6 space-y-4">
            ...
            {shippingSuccess && (
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-455 border border-emerald-105 dark:border-emerald-900 text-xs font-bold rounded-xl flex items-center gap-1.5 animate-scale-in">
                <CheckCircle className="w-4 h-4" /> Address updated successfully!
              </div>
            )}
            ...
            <form onSubmit={handleShippingUpdate} className="space-y-4">
              ...
              <Button type="submit" isLoading={isUpdatingShipping} className="w-full py-2.5 text-sm">
                Save Address Info
              </Button>
            </form>
          </div>
