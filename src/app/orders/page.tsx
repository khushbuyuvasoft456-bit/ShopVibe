...
import { Calendar, CreditCard, ShoppingBag, Package, ChevronRight, ShieldAlert } from "lucide-react";
  ...
  if (!isAuthenticated) {
    return (
      <div className="py-20 max-w-md mx-auto text-center space-y-6">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-zinc-200">Access Denied</h2>
        <p className="text-slate-505">Please sign in to view your orders dashboard.</p>
        <button
      ...
      <div className="pb-20">
        ...
        <div className="text-center py-20 bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-8 max-w-lg mx-auto mt-6 shadow-sm">
          ...
          <p className="text-sm text-slate-505 dark:text-zinc-450 mt-2">
            You haven't checked out any purchases yet. Your completed orders will appear here for you to track and manage.
          </p>
          <Link
            href="/products"
          ...
          >
            ...
            <div className="bg-slate-50/50 dark:bg-zinc-900/40 p-4 sm:p-5 border-b border-slate-100 dark:border-zinc-850 flex flex-wrap gap-4 items-center justify-between">
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm font-semibold">
                <div>
                  <span className="text-slate-450 dark:text-zinc-500 block text-xs">ORDER PLACED</span>
                ...
                <div>
                  <span className="text-slate-450 dark:text-zinc-500 block text-xs">ORDER REFERENCE</span>
                  <span className="text-slate-800 dark:text-zinc-200">{order.id}</span>
                </div>
                <div>
                  <span className="text-slate-455 dark:text-zinc-500 block text-xs">TOTAL PAID</span>
                  <span className="text-slate-950 dark:text-white font-extrabold">${order.total.toFixed(2)}</span>
                </div>
              </div>

              {/* Status Badge */}
              <span className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-full ${getStatusBadgeClass(order.status)}`}>
                {order.status}
              </span>
            </div>
            ...
            <div className="bg-slate-50/20 dark:bg-zinc-900/10 px-4 sm:px-6 py-3 border-t border-slate-50 dark:border-zinc-850 text-xs font-medium text-slate-500 dark:text-zinc-450 flex flex-wrap justify-between items-center gap-2">
              <span className="truncate">
                Shipped to: <strong className="text-slate-700 dark:text-zinc-300">{order.shippingAddress.fullName}</strong> - {order.shippingAddress.street}, {order.shippingAddress.city}
              </span>
