...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const { id, name, price, originalPrice, discount, rating, reviewCount, images, category } = product;
  ...
  return (
    <div className="group relative flex flex-col w-full bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      {/* Product Image Panel */}
      <Link href={`/products/${id}`} className="relative block aspect-square w-full overflow-hidden bg-slate-100 dark:bg-zinc-800">
        <Image
          src={images[0]}
          alt={name}
          fill
